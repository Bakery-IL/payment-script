...
function setup() {
  ...
  if [ ! -z $CHECK_ONLY ]; then
    ...
    echo "found for a total of $TOTAL_STRING ꜩ"
    echo
    exit 0
  fi
...
function simulateTransactions() {
  ...
  log "Simulation returned with $run_response"
...
function main() {
  ...
  if [ -z $SKIP_FUNDING ]; then
    ...
    echo "Send $TOTAL_STRING ꜩ to $ACCOUNT_ADDRESS"
    ...
    echo "Skipping funding ($ACCOUNT_ADDRESS requires $TOTAL_STRING ꜩ)..."
  fi
  echo
